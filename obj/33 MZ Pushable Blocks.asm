...
PushB_Index:	dc.w PushB_Main-PushB_Index		; 0
		...
		dc.w PushB_OnLava-PushB_Index		; 4
...
PushB_Main:	; Routine 0
		...
		move.w	#ArtTile_MZ_Block|Tile_Pal3,obGfx(a0)	; set art tile
	...
	.chkgone:
		...
		btst	#0,2(a2,d0.w)				; has this block been loaded before?
...
PushB_OnLava:	; Routine 4
	...
	.checkGeyser:
		...
		bsr.w	ObjFloorDist				; get distance to floor (and block ID in a1)
		...
		cmpi.w	#$2D2,d0				; is block standing on a 16x16 lava block? (IDs $2D2 and above)
...
PushB_LavaPlatform:
		...
		cmpi.b	#4,obRoutine(a0)			; are we on PushB_OnLava?
		beq.s	.routine4				; if so, branch
		move.b	#4,obRoutine(a0)			; if not, advance to PushB_OnLava
...
.falling:	; ob2ndRout = 4
		...
		asr.w	#3,d0					; divide that speed by 8 ($400/8 = $80)
		move.w	d0,obVelX(a0)				; move block horizontally on lava
		move.b	#4,obRoutine(a0)			; advance to PushB_OnLava
		clr.w	obSubpixelY(a0)				; clear Y-subpixel position (needed for sinking in lava)
